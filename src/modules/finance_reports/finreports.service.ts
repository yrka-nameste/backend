...
export class FinReportsService {
  ...
  async getFinanceSummary(user: AuthUser, period?: string) {
  ...
  async generateFinanceExcel(user: AuthUser, period?: string) {
    const branchId = this.getBranchId(user);
    const normalizedPeriod = normalizePeriod(period);
    const { start, end } = getMonthRange(normalizedPeriod);
    ...
    const previousMonths = await this.getLastSixMonths(
      branchId,
      normalizedPeriod,
    );
  ...
  private async getLastSixMonths(branchId: string, currentPeriod: string) {
  ...
  private fillPaymentsSheet(sheet: ExcelJS.Worksheet, payments: any[]) {
    sheet.columns = [
  ...
  private fillDebtsSheet(sheet: ExcelJS.Worksheet, rows: any[]) {
    sheet.columns = [
  ...
  private fillShopSheet(sheet: ExcelJS.Worksheet, orders: any[]) {
    sheet.columns = [
  ...
  private fillChartsSheet(
    sheet: ExcelJS.Worksheet,
    data: {
      previousMonths: { period: string; income: number }[];
      paidStudentsCount: number;
      debtorsCount: number;
      totalIncome: number;
      totalDebt: number;
    ...
    const maxIncome = Math.max(
      ...data.previousMonths.map((item) => item.income),
      1,
    );
    ...
    const maxValue = Math.max(
      ...chartRows.map((item) => Number(item[1])),
      1,
    );
  ...
  private makeBar(value: number, max: number, width = 32): string {
  ...
  private styleCustomHeader(sheet: ExcelJS.Worksheet, rowNumber: number) {
